...
import {blankColumn, augmentTable, generateReferenceTable, Table} from "../table/tableFns.ts";
...
export default function TruthTable({width}: { width: number }) {
	...
	function exprColumn(expression: string, bindingTable: Table): Table {
		try {
			return bindingTable.map(
				(row) => [Boolean(evalExpression(expression,row))]
			)
		} catch (e) {
	...
	function generateRows(base: Table, extraCols: Table[]): Table {
		...
		return augmentTable(
			...
			extraCols.at(-1) as Table, // Cast okay because of length bailout earlier
		)
	}
